...
freeRoomItem : FreeRoom -> Html Msg
freeRoomItem freeRoom =
    div []
        [ label [] [ text freeRoom.room ]
        , ul [] (List.map (\t -> li [] [ text t ]) freeRoom.times)
        ]


listFreeRooms : FreeRooms -> Html Msg
listFreeRooms freeRooms =
    div [] (List.map (\ft -> freeRoomItem ft) freeRooms.freeTimes)
...
send : Model -> Cmd Msg
send model =
    Http.post ("http://localhost:3000/api/freetimes") (Http.jsonBody (encodeModel model)) freeRoomsUnwrapper
        |> RemoteData.sendRequest
        |> Cmd.map OnResponse


freeRoomsUnwrapper : Decode.Decoder FreeRooms
freeRoomsUnwrapper =
    JDecode.decode FreeRooms
        |> JDecode.required "freeTimes" (Decode.list freeRoomDecoder)


freeRoomDecoder : Decode.Decoder FreeRoom
freeRoomDecoder =
    JDecode.decode FreeRoom
